{ config, pkgs, lib, ... }: {
  ...
  systemd.services.fuchs-mcp-homeassistant = {
    ...
    serviceConfig = {
      ...
      ExecStart       = "${pkgs.uv}/bin/uvx --python ${pkgs.python3}/bin/python3 --with httpx fastmcp run /etc/sojus/fuchs-mcp-homeassistant-server.py --transport streamable-http --host 0.0.0.0 --port 8004";
      Restart         = "on-failure";
      RestartSec      = "30s";
