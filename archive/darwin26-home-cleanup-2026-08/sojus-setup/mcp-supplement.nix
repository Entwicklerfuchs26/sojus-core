{ config, pkgs, lib, ... }:
{
  ...
  systemd.services.sojus-mcp-supplement = {
    description = "Sojus - Supplement MCP Server (Aktivität, Ankündigungen, Polls, Formulare)";
    ...
    serviceConfig = {
      ...
      ExecStart = "${pkgs.uv}/bin/uvx --python ${pkgs.python3}/bin/python3 --with httpx fastmcp run /etc/sojus/mcp-supplement-server.py --transport streamable-http --host 0.0.0.0 --port 8001";
      Restart = "on-failure";
      RestartSec = "30s";
