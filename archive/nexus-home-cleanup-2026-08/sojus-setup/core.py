...
def _auto_classify_tier(tool_name: str) -> int:
    """Auto-classify tier by scanning verb tokens in tool name. Tier 3 > Tier 2 > Tier 1."""
...
def tool_tier(tool_name: str) -> int:
    """Returns security tier: 1=read, 2=write, 3=destructive.
    Checks manual overrides first (tool_tiers.json), then auto-classifies by verb token."""
    ...
    if name in overrides:
        ...
        if val == "content_based":
            return 2  # Fallback ohne Argumente — execute_mcp_tool macht content-based
        return int(val)
    return _auto_classify_tier(name)


# ── EXECUTE_COMMAND CONTENT-BASED TIER ────────────────────────────────────────
...
def _execute_command_tier(arguments: dict) -> int:
    """Content-based tier classification for execute_command."""
