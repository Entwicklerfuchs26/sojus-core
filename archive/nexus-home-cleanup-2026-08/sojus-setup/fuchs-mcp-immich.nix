{ config, pkgs, lib, ... }: {
  ...
  systemd.services.fuchs-mcp-immich = {
    ...
    serviceConfig = {
      ...
      ExecStart       = "${pkgs.uv}/bin/uvx --python ${pkgs.python3}/bin/python3 --with httpx fastmcp run /etc/sojus/fuchs-mcp-immich-server.py --transport streamable-http --host 0.0.0.0 --port 8007";
      Restart         = "on-failure";
      RestartSec      = "30s";
