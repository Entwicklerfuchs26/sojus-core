...
Noch NICHT in core.py eingebunden — erst nach Jonas' OK einschalten.

Architektur:
  tool_groups.json  → welche Keywords → welche Server
  tool_tiers.json   → welche Tools brauchen Bestätigung (Tier 2/3)
  ToolRouter.resolve_servers(query) → Menge der Server-Namen

Verbesserungen gegenüber core.py:
  - Multi-Gruppe: alle passenden Gruppen werden gesammelt (kein break-after-first)
    z.B. "Schick mir Push und leg einen Kalendertermin an" → push + kalender
  - Cache-Invalidierung: mtime-basiert, erkennt Dateiänderungen ohne Neustart
  - resolve_servers() vs. inline keyword-loop: sauber testbar

Integration (ENTWURF — noch nicht scharf):
  In core.py ersetzen:
    _load_tool_groups() + inline get_tools_for_query() Routing-Teil
  Durch:
    from tool_router import ToolRouter
    _router = ToolRouter(MCP_SERVERS, TOOL_GROUPS_FILE, TOOL_TIERS_FILE)
    # In get_tools_for_query():
    servers_to_query = _router.resolve_servers(user_msg)
    # und tool_tier() ersetzen durch:
    tier = _router.tier(tool_name)
"""
...
class ToolRouter:
    ...
    def resolve_servers(self, query: str) -> set[str]:
        ...
        nicht nur die erste. Mehrere Gruppen können gleichzeitig matchen.
        Wenn keine Gruppe matcht → alle Server (wie bisher).
        """
        ...
        for group_name, group_data in groups.items():
            ...
            if any(kw in query_lower for kw in keywords):
                new_servers = set(group_data.get("servers", []))
                matched |= new_servers
                log.info("Tool-Router: Gruppe '%s' → %s", group_name, new_servers)

        if not matched:
            log.info("Tool-Router: kein Keyword-Treffer → alle Server")
            return set(self._servers.keys())

        known = matched & set(self._servers.keys())
        unknown = matched - set(self._servers.keys())
        if unknown:
            log.warning("Tool-Router: unbekannte Server in config: %s", unknown)
        return known

    # ── Tier-Klassifikation ───────────────────────────────────────────────────

    def tier(self, tool_name: str) -> int:
        """1=lesen (default), 2=schreibend, 3=destruktiv/Shell-Execute."""
