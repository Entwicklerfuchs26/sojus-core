{ config, pkgs, lib, ... }:
let
  script = pkgs.writeText "fuchs-discord-server.py"
    (builtins.readFile ../scripts/darwin26/fuchs-discord-server.py);
in {
  ...
  systemd.services.fuchs-discord = {
    ...
    serviceConfig = {
      ...
      ExecStart       = "${pkgs.uv}/bin/uv run --python ${pkgs.python3}/bin/python3 --with 'discord.py' --with fastmcp --with httpx ${script}";
      Restart         = "on-failure";
      RestartSec      = "30s";
